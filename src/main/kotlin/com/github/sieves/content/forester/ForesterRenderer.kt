...
class ForesterRenderer : ApiRenderer<ForesterTile>() {
    ...
    private fun renderHarvests(pBlockEntity: ForesterTile, stack: PoseStack, bufferSource: MultiBufferSource) {
...
//            target.lerp(target, delta + length)
    ...
    private fun renderGrowths(pBlockEntity: ForesterTile) {
    ...
    override fun shouldRenderOffScreen(pBlockEntity: ForesterTile): Boolean {
        return true
    }
    ...
    override fun shouldRender(pBlockEntity: ForesterTile, pCameraPos: Vec3): Boolean {
        return true
    }
}