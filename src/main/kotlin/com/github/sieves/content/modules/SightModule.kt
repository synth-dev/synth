...
class SightModule : ApiTabItem("test".resLoc, BatteryTile::class.java) {
    ...
    companion object {
        ...
        init {
            Registry.Net.SightToggle.clientListener { toggleSightPacket, _ ->
                if (physicalClient)
                    if (toggleSightPacket.enabled) Minecraft.getInstance().options.gamma = 10.0
                    else Minecraft.getInstance().options.gamma = 0.0
                true
            }
        }
        ...
        private fun onServerClick(player: ServerPlayer, tab: ApiTab) {
            Registry.Net.sendToClient(Registry.Net.SightToggle {
                enabled = false
            }, player.uuid)
