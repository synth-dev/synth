...
class FluidsRenderer : ApiRenderer<FluidsTile>() {
    private val texture = ResourceLocation("minecraft", "block/lapis_block")
    private var time = 0f

    override fun render(
        pBlockEntity: FluidsTile,
        pPartialTick: Float,
        stack: PoseStack,
    ...
    private fun drawBeams(stack: PoseStack, bufferSource: MultiBufferSource, tile: FluidsTile) {
        ...
        if (tile.energy.energyStored <= 0) return
        val renderLines =
...
//                )
    ...
    private fun drawPower(stack: PoseStack, bufferSource: MultiBufferSource, tile: FluidsTile) {
    ...
    override fun shouldRenderOffScreen(pBlockEntity: FluidsTile): Boolean {
        return true
    }
